...
class BDQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, num_heads, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        if hasattr(model.output, '__len__') and len(model.output) > num_heads:
            raise ValueError(f'Model "{model}" has more than {num_heads} outputs. Bootstrap DQN expects a model that has exactly {num_heads} outputs.')
        if num_heads > 1:
            for head in model.output:
                if head._keras_shape != (None, self.nb_actions):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        for i in range(self.num_heads):
            if self.num_heads == 1:
                ...
                y_pred = self.model.output[i]
            pred_outputs.append(y_pred)
            y_true = Input(name=f'y_true_{i}', shape=(self.nb_actions,))
            y_true_list.append(y_true)
            mask = Input(name=f'mask_{i}', shape=(self.nb_actions,))
            mask_list.append(mask)
            loss_out = Lambda(clipped_masked_error, output_shape=(1,), name=f'loss_{i}')([y_true, y_pred, mask])
            loss_outputs.append(loss_out)
            # We need to make sure that both loss and output have some function attached.
            # Since Keras only looks for Callable, dict, or string we need to have a Dict[str, Callable] data structure
            losses_dict[f'loss_{i}'] = lambda y_true, y_pred: y_pred
            losses_dict[f'output_{i}'] = lambda y_true, y_pred: K.zeros_like(y_pred)
        ins = [self.model.input] if type(self.model.input) is not list else self.model.input
        trainable_model = Model(inputs=ins + y_true_list + mask_list, outputs=loss_outputs + pred_outputs)
        combined_metrics = {trainable_model.output_names[-1]: metrics}
        trainable_model.compile(optimizer=optimizer, loss=losses_dict, metrics=combined_metrics)
...
class DQNAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        # using a custom Lambda layer that computes the loss. This gives us the necessary flexibility
        # to mask out certain parameters by passing in multiple inputs to the Lambda layer.
        y_pred = self.model.output # Vector of Q-Values. If argmax is used, you get the predicted action.
        y_true = Input(name='y_true', shape=(self.nb_actions,)) #The target value for the regression problem; 
        mask = Input(name='mask', shape=(self.nb_actions,)) # Zero mask used hide all other actions except the action in the <s,a,r,s'> tuple
